...
public record PlayerInfo(String uuid, String name, Map<String, Ranking> rankings, String region, int points,
    ...
    public static CompletableFuture<PlayerInfo> get(HttpClient client, String name) {
        return search(client, name);
    }

    public static CompletableFuture<PlayerInfo> search(HttpClient client, String query) {
        String endpoint = "https://www.mcpvp.com/tiers/search?q=" + query + "&kit=overall&include_retired=0";
        final HttpRequest request = HttpRequest.newBuilder(URI.create(endpoint)).GET().build();

        return client.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                .thenApply(HttpResponse::body)
                .thenApply(s -> {
                    com.google.gson.JsonObject obj = TierTagger.GSON.fromJson(s, com.google.gson.JsonObject.class);
                    com.google.gson.JsonArray players = obj.getAsJsonArray("players");
                    if (players == null || players.isEmpty()) return null;
                    com.google.gson.JsonObject player = players.get(0).getAsJsonObject();

                    String uuidStr = player.get("uuid").getAsString();
                    String name = player.get("name").getAsString();
                    String region = player.get("region").getAsString();
                    int points = player.get("points").getAsInt();
                    boolean retired = player.get("retired").getAsBoolean();

                    Map<String, Ranking> rankings = new HashMap<>();
                    com.google.gson.JsonObject kitRanks = player.getAsJsonObject("kitRanks");
                    com.google.gson.JsonObject kitRetired = player.getAsJsonObject("kitRetired");

                    if (kitRanks != null) {
                        for (String kit : kitRanks.keySet()) {
                            String rankStr = kitRanks.get(kit).getAsString();
                            boolean isRetired = kitRetired != null && kitRetired.has(kit) && kitRetired.get(kit).getAsBoolean();

                            int tier = 0;
                            int pos = 0;
                            if (rankStr.length() >= 3) {
                                pos = rankStr.charAt(0) == 'H' ? 0 : 1;
                                try {
                                    tier = Integer.parseInt(rankStr.substring(2));
                                } catch (NumberFormatException ignored) {}
                            }

                            rankings.put(kit, new Ranking(tier, pos, tier, pos, 0, isRetired));
                        }
                    }

                    return new PlayerInfo(uuidStr, name, rankings, region, points, 0, List.of(), false);
                })
                .whenComplete((i, t) -> {
